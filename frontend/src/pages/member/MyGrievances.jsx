...
export default function MyGrievances() {
  ...
  const loadGrievances = async () => {
    try {
      ...
      const data = await grievanceAPI.myGrievances();
      setGrievances(data);
    } catch (err) {
      setError("Unable to load grievances. Please login again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredGrievances = useMemo(() => {
    ...
    return grievances.filter((item) => {
      const categoryText = formatCategory(item.category).toLowerCase();
  ...
  return (
    <section style={page}>
      <div style={topRow}>
        <div>
          <p style={tag}>My Grievances</p>

          <h1 style={heading}>Track your submitted concerns</h1>

          <p style={subText}>
            Search your tickets, filter by status or priority, and track admin
            updates for every grievance.
          </p>
        </div>

        <Link to="/member/submit-grievance" style={button}>
          + New Grievance
        </Link>
      </div>
        ...
        {loading ? (
          ...
          <div style={emptyBox}>
            ...
            <p style={{ margin: 0, color: "#65535a", lineHeight: "1.7" }}>
              You have not submitted any grievance yet. Click on New Grievance
              to submit your first concern.
            </p>
          </div>
        ) : filteredGrievances.length === 0 ? (
          <div style={emptyBox}>
            <h3 style={{ margin: "0 0 8px", color: "#993556" }}>
              No matching grievance
            </h3>

            <p style={{ margin: 0, color: "#65535a", lineHeight: "1.7" }}>
              No grievance matched your search or filters. Clear filters and
              try again.
            </p>
          </div>
        ) : (
